...
detects_study <- read.csv("detects_Juv_Green_Sturgeon_2017.csv", stringsAsFactors = F)
...
endtime <- as.Date(c(Sys.time()))#, max(tower_detects$first_detect)+60*60*24)))
...
barp <- barplot(height= daterange2$unique_tags, col="gray",  names.arg= daterange2$Day, xlab = "", ylab = "Number of fish arrivals per day", ylim = c(0,max(daterange2$unique_tags, na.rm = T)*1.5), las = 2, xlim=c(0,nrow(daterange2)+1.5), cex.lab = 1.5, yaxt = "n", xaxt = "n")
ybreaks <- if(max(daterange2$unique_tags, na.rm = T) < 4) {max(daterange2$unique_tags, na.rm = T)} else {5}
xbreaks <- if(length(barp) > 10) {seq(1, length(barp), 2)} else {1:length(barp)}
