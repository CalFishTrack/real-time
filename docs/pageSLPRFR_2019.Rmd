...
if (nrow(study_tagcodes) == 0){
  ...
  study_tagcodes$release <- "1"
  study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2019-04-12"), "release"] <- "2"
...
detects_study <- read.csv("detects_ColemanAltRelStrat2019.csv", stringsAsFactors = F)
...
if (nrow(detects_butte) == 0){
  ...
  legend(x ='topleft', legend = colnames(daterange2)[1:ncol(daterange2)-1], fill= rainbow(rel_num), horiz = T, title = "Releas")
