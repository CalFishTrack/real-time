...
# **Mill and Deer Creek wild Chinook salmon, Spring Releases**
...
Telemetry Study Template for this study can be found [here](https://github.com/CalFishTrack/real-time/blob/master/data/Telemetry_Study_Summary_Deer_Creek_Spring-Run.pdf?raw=true)
...
study_tagcodes <- tagcodes[tagcodes$StudyID == "MillDeerCK_CH_Wild_Spring2020",]
...
if (nrow(study_tagcodes) == 0){
  ...
  study_tagcodes$Release <- "Deer Creek release"
  study_tagcodes[study_tagcodes$Rel_loc == "Mill_Creek", "Release"] <- "Mill Creek release"
...
detects_study <- fread(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products\\Study_detection_files\\detects_MillDeerCK_CH_Wild_Spring2020.csv", sep = ""), colClasses = c(DateTime_PST = "character", RelDT = "character"))
