...
```{r print figure of fish detections at Butte, message = FALSE, warning = F, fig.height = 6, fig.width = 10, out.extra='style="background-color: #000000; padding:3px"', fig.cap="2.1 Detections at Butte City Bridge versus Sacramento River flows at Butte City"}
...
detects_study <- fread(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products\\Study_detection_files\\detects_ColemanLateFall_2020.csv", sep = ""), colClasses = c(DateTime_PST = "character", RelDT = "character"))
...
if (nrow(detects_study) == 0){
  ...
  ## remove old benicia record for this studyID
  Delta <- Delta[!Delta$StudyID == unique(study_tagcodes$StudyID),]
...
if (nrow(detects_study) == 0){
  ...
  ## Now only keep beacon by day for days since fish were released, and up to today or taglife
 ...
 kable(crosstab, align = "c", escape = F, booktabs = T) %>%
  kable_styling() %>%
  #row_spec(0, angle = -45) %>%
  column_spec(column = 1:12,width_min = "50px",border_left = T, border_right = T) %>%
  column_spec(1, bold = T, width_min = "70px")%>%
