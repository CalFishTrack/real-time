...
if (nrow(study_tagcodes) == 0){
  ...
  study_tagcodes$Release_week <- "Week 1"
  #study_tagcodes[study_tagcodes$RelDT < as.POSIXct("2018-11-18"), "Release_week"] <- "Week 1"
  #study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2018-11-18") & study_tagcodes$RelDT < as.POSIXct("2018-11-25"), "Release_week"] <- "Week 1"
  study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2018-12-02") & study_tagcodes$RelDT < as.POSIXct("2018-12-09"), "Release_week"] <- "Week 2"
  study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2019-04-14") & study_tagcodes$RelDT < as.POSIXct("2019-04-20"), "Release_week"] <- "Week 3"
  study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2019-04-20") & study_tagcodes$RelDT < as.POSIXct("2019-04-27"), "Release_week"] <- "Week 4"
  study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2019-04-27") & study_tagcodes$RelDT < as.POSIXct("2019-05-04"), "Release_week"] <- "Week 5"
  study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2019-05-04") & study_tagcodes$RelDT < as.POSIXct("2019-05-11"), "Release_week"] <- "Week 6"
...
detects_study <- read.csv("detects_DeerCk-SH-Wild-F2019.csv", stringsAsFactors = F)
detects_study$DateTime_PST <- as.POSIXct(detects_study$DateTime_PST, format = "%Y-%m-%d %H:%M:%S", "Etc/GMT+8")

if(nrow(detects_study)>0){
  detects_study <- merge(detects_study, study_tagcodes[,c("TagID_Hex", "RelDT", "StudyID", "Release_week", "tag_life")], by.x = "TagCode", by.y = "TagID_Hex")
}
...
if (nrow(detects_tower) == 0){
  ...
  rels <- unique(study_tagcodes[study_tagcodes$StudyID == unique(detects_tower$StudyID), "Release_week"])
  rel_num <- length(rels)
  rels_no_detects <- as.character(rels[!(rels %in% unique(detects_tower$Release_week))])

  tagcount <- aggregate(list(unique_tags = detects_tower$TagCode), by = list(Day = detects_tower$Day, Release_week = detects_tower$Release_week ), FUN = function(x){length(unique(x))})
...
if (nrow(detects_benicia)>0) {
  ...
  rels <- unique(study_tagcodes[study_tagcodes$StudyID == unique(detects_benicia$StudyID), "Release_week"])
  rel_num <- length(rels)
  rels_no_detects <- as.character(rels[!(rels %in% unique(detects_benicia$Release_week))])
  
  tagcount <- aggregate(list(unique_tags = detects_benicia$TagCode), by = list(Day = detects_benicia$Day, Release_week = detects_benicia$Release_week ), FUN = function(x){length(unique(x))})
  tagcount1 <- dcast(tagcount, Day ~ Release_week)
...
```{r print table of survival to Tower, echo = FALSE, message = FALSE, results= "asis", warning=FALSE}
...
if (nrow(detects_study) == 0){
  ...
  tag_stats <- aggregate(list(First_arrival = detects_study$DateTime_PST), 
                         by= list(general_location = detects_study$general_location),
                         FUN = min)
  tag_stats <- merge(tag_stats, 
                     aggregate(list(Mean_arrival = detects_study$DateTime_PST), 
                         by= list(general_location = detects_study$general_location),
                         FUN = mean), 
                     by = c("general_location"))
  tag_stats <- merge(tag_stats, 
                     aggregate(list(Fish_count = detects_study$TagCode), 
                         by= list(general_location = detects_study$general_location), 
                         FUN = function(x) {length(unique(x))}), 
                     by = c("general_location"))
  ...
  for (j in sort(unique(study_tagcodes$Release_week))) {
    ...
    if(nrow(detects_study[detects_study$Release_week == j,]) > 0 ) {
    
      temp <- detects_study[detects_study$Release_week == j,]
