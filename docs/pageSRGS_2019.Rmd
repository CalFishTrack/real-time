...
if (nrow(detects_study) == 0){
  ...
  detects_study$DateTime_PST <- as.POSIXct(detects_study$DateTime_PST, format = "%Y-%m-%dT%H:%M:%S", "Etc/GMT+8")
...
if (nrow(test) == 0){
  ...
  test$DateTime_PST <- as.POSIXct(test$DateTime_PST, format = "%Y-%m-%dT%H:%M:%S")
