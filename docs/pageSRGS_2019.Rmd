...
Telemetry Study Template for this study can be found [here](https://github.com/CalFishTrack/real-time/blob/master/data/Telemetry Study Summary Framework_Sac_River_Green_Sturgeon.pdf?raw=true)
...
```{r print figure of fish detections, message = FALSE, fig.height = 6, fig.width = 10}
setwd(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products", sep = ""))

library(CDECRetrieve)

detects_study <- read.csv("detects_Juv_Green_Sturgeon_2018.csv", stringsAsFactors = F)
...
```{r print table of fish detections}
setwd(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products", sep = ""))

library(data.table)

test <- fread("detects_Juv_Green_Sturgeon_2018.csv", stringsAsFactors = FALSE)
