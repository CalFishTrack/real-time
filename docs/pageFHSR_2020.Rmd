...
htmltools::img(src = knitr::image_uri("../data/feather-river-hatchery.jpeg"))#, 
               #alt = 'logo', 
...
study_tagcodes <- tagcodes[tagcodes$StudyID == "FR_Spring_2020",]
...
detects_study <- fread(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products\\Study_detection_files\\detects_FR_Spring_2020.csv", sep = ""))
...
if (nrow(detects_benicia) == 0){
  ...
  inp <- cbind(inp, inp2)
  groups <- as.character(sort(unique(inp$Release)))
  test3$Release <- factor(test3$Release, levels = groups)
  ...
  if(length(groups) > 1){
  ## make sure factor levels have a release that has detections first. if first release in factor order has zero detectins, model goes haywire
    inp.df <- data.frame(ch = as.character(inp$inp_final), freq = 1, rel = factor(inp$Release, levels = names(sort(table(test3$Release),decreasing = T))), stringsAsFactors = F)
  ...
  WR.surv <- cbind(Release = c("ALL", names(sort(table(test3$Release),decreasing = T))), WR.surv)
...
if (nrow(detects_study) == 0){
  ...
  ## remove old benicia record for this studyID
  Delta <- Delta[!Delta$StudyID == unique(Delta$StudyID),]
