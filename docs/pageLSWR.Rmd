...
legend(x ='top', legend = colnames(daterange2[,1:ncol(daterange2)-1]), fill= rainbow(rel_num), horiz = T, title = "Release Group")
...
```{r print table of fish detections, echo = FALSE, message = FALSE, results= "asis"}
setwd(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products", sep = ""))
...
if (nrow(test) == 0){
  ...
  for (j in as.character(unique(study_tagcodes$RelDT))) {
    ...
    if(nrow(test[test$RelDT == j,]) > 0 ) {
      ...
      print(kable(tag_stats1, row.names = F, 
            caption = paste("Detections for",as.character(j),"release group", sep = " "),
            "html") %>%
          kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position = "left"))
      
    } else {
      cat("\n\n\\pagebreak\n")
      print(paste("No detections for release group",j,"yet", sep=" "), quote = F)
