...
htmltools::img(src = knitr::image_uri("../data/NF_Battle.jpg"))#, 
               #alt = 'logo', 
...
# *Hatchery-origin Battle Creek winter-run Chinook salmon*
...
study_tagcodes <- tagcodes[tagcodes$StudyID == "BC_Jumpstart_2019",]
...
if (nrow(study_tagcodes) == 0){
  ...
  study_tagcodes$tag_life <- NA 
  study_tagcodes[study_tagcodes$PRI_nominal < 6, "tag_life"] <- 30
  study_tagcodes[study_tagcodes$PRI_nominal < 11 & study_tagcodes$PRI_nominal > 9, "tag_life"] <- 60
...
detects_study <- read.csv("detects_BC_Jumpstart_2019.csv", stringsAsFactors = F)
...
if (nrow(detects_butte) == 0){
  ...
  endtime <- min(as.Date(c(Sys.time())), max(as.Date(detects_butte$RelDT)+detects_butte$tag_life))
  ...
  rels <- unique(study_tagcodes[study_tagcodes$StudyID == unique(detects_butte$StudyID), "Release_week"])
  rel_num <- length(rels)
  rels_no_detects <- as.character(rels[!(rels %in% unique(detects_butte$Release_week))])

  tagcount <- aggregate(list(unique_tags = detects_butte$TagCode), by = list(Day = detects_butte$Day, Release_week = detects_butte$Release_week ), FUN = function(x){length(unique(x))})
