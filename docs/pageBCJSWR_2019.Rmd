...
detects_study <- read.csv("C:/Users/field/Desktop/Real-time data massaging/products/Study_detection_files/detects_BC_Jumpstart_2019.csv", stringsAsFactors = F)
detects_study$DateTime_PST <- as.POSIXct(detects_study$DateTime_PST, format = "%Y-%m-%d %H:%M:%S", "Etc/GMT+8")

if(nrow(detects_study)>0){
  detects_study <- merge(detects_study, study_tagcodes[,c("TagID_Hex", "StudyID", "Release", "RelDT", "tag_life")], by.x = "TagCode", by.y = "TagID_Hex")
}
...
```{r print figure of fish detections at Tower, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}
