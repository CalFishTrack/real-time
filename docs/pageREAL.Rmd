...
test <- aggregate(detects_beacon1$DateTime_PST, by = list(recv = detects_beacon1$recv, hour = format(detects_beacon1$DateTime_PST, format = "%Y-%m-%d %H")), FUN = length)
test <- merge(test, gap_percent[, c("recv", "recv_start")])
test <- test[test$hour > test$recv_start,]
gap_percent <- merge(gap_percent, aggregate(list(hour_count = test$hour), by = list(recv = test$recv), FUN = length), by = "recv", all.x = T)
...
test <- test[!test$recv %in% c("17131", "17136", "17139", "17140", "17142"),]
test$recv <- factor(test$recv)

## Estimate hourly detection efficiency
test <- merge(test,gen_locs[,c("recv", "beacon_PRI")], by = "recv", all.x = T)
test$expected <- 60*60/test$beacon_PRI
test$hourly_efficiency <- round(test$x/test$expected,3)*100

## And for last week
last_week <- test[which(as.POSIXct(test$hour, format = "%Y-%m-%d %H") > Sys.time()-60*60*24*2),]

## Finally, make a table with all-time and last week's mean hourly detection efficiency
efficiency <- aggregate(list(all_time_efficiency = test$hourly_efficiency), by = list(recv = test$recv), FUN = mean)
...
test <- merge(test, gen_locs[,c("recv", "location", "rkm")], by = "recv")
test <- test[order(test$rkm, decreasing = T),]

par(mar=c(5.1,2.1,2.1,7.1))
plot(as.POSIXct(test$hour, format = "%Y-%m-%d %H"), test$recv, yaxt = "n", xlab = "Total time receiver has been deployed and operational over whole study")
axis(4, labels = paste(unique(test$recv),unique(test$location), sep = " "), at = 1:length(unique(test$recv)), las = 2)
points(gap_percent[!gap_percent$recv %in% c("17131", "17136", "17139", "17140", "17142"), "recv_start"],1:length(unique(test$recv)), col = "red", pch = 15)
...
## And for last week
last_week$recv <- factor(last_week$recv)
last_week <- merge(last_week, gen_locs[,c("recv", "location", "rkm")], by = "recv")
last_week <- last_week[order(last_week$rkm, decreasing = T),]
...
axis(4, labels = paste(unique(last_week$recv),unique(last_week$location), sep = " "), at = 1:length(unique(last_week$recv)), las = 2)
points(gap_percent[!gap_percent$recv %in% c("17131", "17136", "17139", "17140", "17142"), "recv_start"],1:length(unique(last_week$recv)), col = "red", pch = 15)
