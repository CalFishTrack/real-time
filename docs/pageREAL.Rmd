...
for (k in unique(tagcodes$StudyID)){
  ...
  if(nrow(temp_detects) > 0){
    ...
    ## Now mark tag detections that have a detection both before and after from the same tag at the same gen loc within 2 hours (within 2 hours before and 2 hours after)
    temp_detects$before_and_after_valid <- NA
    temp_detects[is.na(temp_detects$time_to_next) == F & is.na(temp_detects$time_from_previous) == F & temp_detects$time_to_next < 1 & temp_detects$time_from_previous < 1,"before_and_after_valid"] <- 1
    ## Now pull out only these detections that are part of at least 3 consecutive detections that are no more than 2 hours apart
