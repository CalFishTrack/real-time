...
if (nrow(ATS_files) > 0) {
  ...
  if (length(detects_temp) > 0){
   ## Now remove prefix and suffix of TagCode
    ...
    ## Make sure no rows are all NA, this has been happening on rare occasions
    detects_temp <- detects_temp[!is.na(detects_temp$recv),]
...
for (k in unique(tagcodes$StudyID)){
  ...
  if(nrow(temp_detects) > 0){
    ...
    ## Now mark tag detections that have a detection both before and after from the same tag at the same gen loc within 14 minute span (this seems like a good cutoff) (within 7 minutes before and 7 minutes after)
    temp_detects$before_and_after_valid <- NA
    temp_detects[is.na(temp_detects$time_to_next) == F & is.na(temp_detects$time_from_previous) == F & temp_detects$time_to_next < 0.1167 & temp_detects$time_from_previous < 0.1167,"before_and_after_valid"] <- 1
