...
Tekno_SUM_processed_files <- fread("Tekno_SUM_processed_files.csv", stringsAsFactors = F, sep = ",")
...
detects <- fread("detects_final.csv", stringsAsFactors = FALSE,colClasses = c(DateTime_PST = "character", DateTime_Orig = "character"))
detects_beacon <- fread("detects_beacon.csv", stringsAsFactors = FALSE,colClasses = c(DateTime_PST = "character", DateTime_Orig = "character"))
...
TECKNO_all_files <- as.data.frame(list(x=dir(pattern = "*.txt", path = "C:/Users/field/Desktop/Tekno_data/Dropbox (Teknologic LLC)/Apps/SRCOM", full.names = T, recursive = T)), stringsAsFactors = F)
...
TECKNO_files$start_nchar_date <- nchar(TECKNO_files$x)-23
TECKNO_files$stop_nchar_date <- nchar(TECKNO_files$x)-14
## Also, remove all the older files that google drive messed up
TECKNO_files$dayes <- as.Date(substr(TECKNO_files$x, TECKNO_files$start_nchar_date, TECKNO_files$stop_nchar_date),"%Y_%m_%d")
TECKNO_files <- TECKNO_files[TECKNO_files$dayes > "2020-12-31",]
TECKNO_files$dayes <- NULL
TECKNO_files$start_nchar_date <- NULL
TECKNO_files$stop_nchar_date <- NULL
...
## Find all Tekno SUM files for processing
Tekno_SUM_all_files <- as.data.frame(list(x=dir(pattern = ".SUM", path = "C:/Users/field/Desktop/Tekno_SUM_files", full.names = T)), stringsAsFactors = F)
## Select only newest SUM files for massaging
Tekno_SUM_files <- as.data.frame(list(x=Tekno_SUM_all_files[!Tekno_SUM_all_files$x %in% Tekno_SUM_processed_files$x,]), stringsAsFactors = F)


#***********************************************************************************************
# 4. Data massage new detections if in standard format. Do this for ATS, Tekno, Tekno_SUM and UCD separately ####
#***********************************************************************************************
  ...
  for (i in 1:nrow(ATS_files)) {
    ...
    if (length(end_final) == 0 | file.info(ATS_files[i,"x"])$size == 0 | inherits(trytest, "try-error") | nrow(test[is.na(test$DateTime)==F,]) == 0) {
      ...
      if(ATS_files[i, "shortname"] %in% corrupted_downloads$V1){
        corrupted_downloads[which(corrupted_downloads$V1 == ATS_files[i, "shortname"]), "download_attempts"] <- 
          as.numeric(corrupted_downloads[which(corrupted_downloads$V1 == ATS_files[i, "shortname"]), "download_attempts"])+1
      }else{
      ...
      if(ATS_files[i, "shortname"] %in% corrupted_downloads$V1){
        corrupted_downloads <- corrupted_downloads[!which(corrupted_downloads$V1 == ATS_files[i, "shortname"]),] 
      }
      ## Now data massage
...
if (nrow(TECKNO_files) > 0) {
  ...
  for (i in 1:nrow(TECKNO_files)) {
    ...
    if (inherits(res, "try-error")) {
      corrupted_downloads <- rbind(corrupted_downloads,cbind(V1 = TECKNO_files[i,"shortname"], download_attempts = 5, filename = TECKNO_files[i,"x"]))
...
#***********************************************************************************************
#### 4.d. Now data massage Tekno SUM files ####
## These are the root Tekno files from which hourly files are generated
## When a receiver is manually downloaded in person, these are the files that come off the recv
#***********************************************************************************************
...
if (nrow(Tekno_SUM_files) > 0) {
  
  
  ## First, find shortname for files
  shortname <- regexpr(Tekno_SUM_files$x, pattern = "Tekno_SUM_files")[1]
  Tekno_SUM_files$shortname  <- substr(Tekno_SUM_files$x, shortname+16, nchar(Tekno_SUM_files$x))
  
  downloads_new4 <- data.frame()
  
  for (i in 1:nrow(Tekno_SUM_files)) {
    test <- try(read.csv(Tekno_SUM_files[i,"x"], header = T, skip =8, stringsAsFactors = F ), silent = T)
    if (inherits(test, "try-error")) {
      next
    } else {
      hour <- test[1,"Date.Time"]
      recv <- gsub("-", "", test[1,"Serial.Number"])
      test$Date.Time1 <- as.POSIXct(test$Date.Time, "%m/%d/%Y %H:%M:%OS", tz = "GMT")
      downloads_new4 <- rbindlist(list(downloads_new4, data.frame(list(filename = Tekno_SUM_files[i,"shortname"], recv=recv, hour=hour, sun_volts = NA, bat_volts = NA, end = max(test$Date.Time1), start = min(test$Date.Time1)), stringsAsFactors = F)))
    }
  }

  
  ## Now change all times to PST to match ATS receivers
  attributes(downloads_new4$start)$tzone <- "Etc/GMT+8"
  attributes(downloads_new4$end)$tzone <- "Etc/GMT+8"
  
  downloads_new_all <- rbindlist(list(downloads_new_all, downloads_new4))
  ...
  for (i in 1:nrow(Tekno_SUM_files)) {
    res <- try(read.csv(Tekno_SUM_files[i,"x"], header = F), silent = T) 
    if (inherits(res, "try-error")) {
      corrupted_downloads <- rbind(corrupted_downloads,cbind(V1 = Tekno_SUM_files[i,"shortname"], download_attempts = 5, filename = Tekno_SUM_files[i,"x"]))
      ## Also make corrupted downloads more palateable
      next
    } else {
      test <- read.csv(Tekno_SUM_files[i,"x"], header = T, skip =8, stringsAsFactors = F )
      test$filename <- Tekno_SUM_files[i,"shortname"]
      detects_temp <- rbindlist(list(detects_temp, test[,c("Serial.Number", "Date.Time", "TagCode", "Temp", "filename")]))
    }
  }

  
  colnames(detects_temp) <- c("recv", "DateTime_Orig", "TagCode", "Temp", "filename")
  ## Make DateTime_Orig in same format as other receiver outputs
  ## Make a POSIX class, note that TECKNO detections are in GMT, will need to convert to local time
  detects_temp$DateTime_PST <- as.POSIXct(detects_temp$DateTime_Orig, "%m/%d/%Y %H:%M:%OS", tz = "GMT")
  ## Make DateTime_Orig in same format as other receiver outputs
  detects_temp$DateTime_Orig <- format(detects_temp$DateTime_PST, "%Y-%m-%d %H:%M:%OS6")
  ## convert to PST
  attributes(detects_temp$DateTime_PST)$tzone <- "Etc/GMT+8"
  ## remove dash from recv name
  detects_temp$recv <- gsub("-", "", detects_temp$recv)
  ## Now remove prefix and suffix of TagCode
  detects_temp$TagCode <- substr(detects_temp$TagCode, start = 4, stop = 7)
  ## Remove "ffff" detections
  detects_temp <- detects_temp[!detects_temp$TagCode == "ffff",]
  
  
  #***********************************************************************************************
  # 5.d. Split detections based on whether they are from any of the known beacons, or other, append to respective dataset. Do this for Tekno files now ####
  #***********************************************************************************************
  ...
  #***********************************************************************************************
  # 6.d. Remove multipath detections for beacon detects. Do this for Teckno files now ####
  #***********************************************************************************************
  ...
  #***********************************************************************************************
  # 7.d. Bind new detection data to historic data. Do this for Tekno files now ####
  #***********************************************************************************************
...
#***********************************************************************************************
#### 8. Remove duplicate detections from both detects and beacon detects, and remove detections that are happening in future ####
#### future detections sometimes happen on ATS receivers a few minutes before midnight ####
#***********************************************************************************************
...
## remove future detects. If anyone accidentally deploys an ATS receiver in GMT, this could cause problems, but perhaps that is good, it avoids having bad detections added to the website and the issue will become obvious on the realtime efficiency heatmap
detects <- detects[detects$DateTime_PST < Sys.time(),]
detects_beacon <- detects_beacon[detects_beacon$DateTime_PST < Sys.time(),]
...
for (k in unique(tagcodes$StudyID)){
  ...
  #### 16.b. Second: “PRI filter”: the PRI from the previous detection and the PRI to the next detection on an individual receiver is estimated for each detection of a potentially valid fish visit. At least one of the PRIs must be within 20% of the tag’s programmed PRI AND the standard deviation of the two PRIs must be smaller than 0.025. ####
  ...
  if(nrow(temp_detects) > 0){  
    ...
    temp_detects <- temp_detects[temp_detects$visit_num %in% detects_per_visit_num[detects_per_visit_num$x > 3, "visit_num"],]
  }
...
Tekno_SUM_processed_files <- rbindlist(list(Tekno_SUM_processed_files, data.frame(Tekno_SUM_files[,"x"])))
Tekno_SUM_processed_files$x <- as.character(Tekno_SUM_processed_files$x)
fwrite(Tekno_SUM_processed_files, "Tekno_SUM_processed_files.csv", row.names = F, quote = T)  
...
last_download <- aggregate(list(last_download = downloads$end), by = list(location = downloads$location), FUN = max, na.rm =T)
...
gen_loc_stats <- merge(gen_loc_stats, aggregate(list(recv_start = gen_locs$start), by = list(location = gen_locs$location), min, na.rm = T), by = "location")

gen_loc_stats <- merge(gen_loc_stats, efficiency, by = "location", all.x= T)

gen_loc_stats$Active <- "No"
gen_loc_stats[which(as.numeric(difftime(Sys.time(), gen_loc_stats$last_download, units = "days")) < 7), "Active"] <- "Yes"
...
```{r print heatmap of detection efficiency, warning=FALSE, message=FALSE, fig.width = 10, fig.height = 10}
...
if (nrow(volt.files_2weeks) >0){
  ...
  volt.files_2weeks <- merge(volt.files_2weeks, IPs[,c("RECV", "TCP_IP")], by.x = "IP", by.y = "TCP_IP", all.x = T)
  
  volt.files_2weeks <- volt.files_2weeks[,c("Datetime", "Volts", "RECV","IP")]
} else{
  volt.files_2weeks <- data.frame("Datetime" = as.character(), "Volts" = numeric(), "RECV" = character(),"TCP_IP" = character(), stringsAsFactors = F)
