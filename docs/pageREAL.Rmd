...
if (nrow(TECKNO_files) > 0) {
  ...
  detects_new$DateTime_PST <- paste(detects_new$DateTime_Orig, substr(formatC(as.numeric(detects_new$dec_seconds),format='f', digits=6 ),2,8), sep = "")
  ## Make a POSIX class, note that TECKNO detections are in GMT, will need to convert to local time
  detects_new$DateTime_PST <- as.POSIXct(detects_new$DateTime_PST, format = "%Y-%m-%d %H:%M:%OS", tz = "GMT")
  ...
  detects <- rbind(detects, detects_new[,c("recv", "DateTime_Orig", "dec_seconds", "TagCode", "DateTime_PST", "location", "general_location", "latitude", "longitude")])
