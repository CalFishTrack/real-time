...
```{r bring in data, massage, mention last hour download, echo=FALSE, warning=FALSE}
...
for (i in 2:nrow(downloads)){
  ...
  if (downloads[i-1, "recv"] != downloads[i, "recv"] & difftime(Sys.time(), downloads[i-1, "start"], units = "hours") > 1.25) {
    downloads[i-1,"gapstart"] <- downloads[i-1, "end"]
...
for (i in 1:length(files)) {
  ...
  if (length(end1) + length(end2) == 0) {
    ...
    test1 <- test[!test$TagCode == "",]
    test2 <- test1[!test1$TagCode == " TagCode",]
...
## Tag Detection statistics
``` {r print tagstats, echo=FALSE, warning=FALSE}
