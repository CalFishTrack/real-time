...
for (k in unique(tagcodes$StudyID)){
  ...
  if(nrow(temp_detects) > 0){
    ...
    temp_detects$visit_num <- 1
    for (i in 2:nrow(temp_detects)){
      if(temp_detects[i, "before_and_after_valid"] == 0 & temp_detects[i-1, "before_and_after_valid"] == 0) {
        temp_detects[i, "visit_num"] <- temp_detects[i-1, "visit_num"] +1 
      }else{
        temp_detects[i, "visit_num"] <- temp_detects[i-1, "visit_num"]
      }
    }
  ...
  temp_detects <- temp_detects[, !names(temp_detects) %in% c("before_and_after_valid", "visit_num")]
