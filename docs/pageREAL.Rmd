...
gen_locs$recv <- as.numeric(gen_locs$recv)
...
gap_percent <- merge(gap_percent, aggregate(list(hour_count = test$hour), by = list(recv = test$recv), FUN = length), by = "recv")
...
## Now make plots of receiver beacon detections through time. First remove the receivers without beacons
...
axis(4, labels = unique(test$recv), at = 1:length(unique(test$recv)), las = 2)
...
par(mar=c(5.1,2.1,2.1,4.1))
plot(as.POSIXct(last_week$hour, format = "%Y-%m-%d %H"), last_week$recv, yaxt = "n", xaxt = "n", xlab =  "Total time receiver has been deployed and operational over last week")
axis(4, labels = unique(last_week$recv), at = 1:length(unique(last_week$recv)), las = 2)
axis(1, at = unique(as.POSIXct(last_week$hour, format = "%Y-%m-%d %H")), labels = format(unique(as.POSIXct(last_week$hour, format = "%Y-%m-%d %H")), "%d %H"), las = 2)
...
## And for last week
last_week <- test[which(as.POSIXct(test$hour, format = "%Y-%m-%d %H") > Sys.time()-60*60*24*7),]
