...
for (k in unique(tagcodes$StudyID)){
  ...
  temp_detects <- as.data.frame(detects_filtered[detects_filtered$TagCode %in% studycodes$TagID_Hex,])
...
```{r print voltage for ATS receivers,fig.height = 30}
...
IPs <- read.csv("C:/Users/Trailer/Desktop/realtime_stuff/ATS_Realtime_download_script/IP_addresses.csv", stringsAsFactors = F)
volt.files$IP <- NULL
volt.files$Volts <- NULL

for (i in 1:nrow(volt.files)){
  test <- read.csv(volt.files[i,"path"], skip = 3, stringsAsFactors = F)
  volt.files[i,"Volts"] <- as.numeric(substr(test[1,],11,15))
  volt.files[i,"IP"] <- substr(test[2,],5,nchar(test[2,]))
}

volt.files <- merge(volt.files, IPs[,c("RECV", "TCP_IP")], by.x = "IP", by.y = "TCP_IP")

ggplot(volt.files, aes(x = Datetime, y = Volts)) + 
  geom_line() + 
  geom_point() +
  ylim(9,15) +
  facet_grid(RECV ~ .) + 
