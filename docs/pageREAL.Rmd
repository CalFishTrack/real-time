...
if (nrow(ATS_files) > 0) {
  ...
  if (length(detects_temp) > 0){
    ...
    detects_beacon_new <- detects_temp[detects_temp$TagCode %in% c(unique(beacon_deployments$beacon), "006F", "006D", "FCAA"),]
...
## Also do per day
beacon_by_day <- aggregate(detects_beacon1$DateTime_PST, by = list(recv = detects_beacon1$recv, TagCode = detects_beacon1$TagCode, day = format(detects_beacon1$DateTime_PST, format = "%Y-%m-%d")), FUN = length)
beacon_by_day$day <- as.Date(beacon_by_day$day)
beacon_by_day$beacon <- as.character(NA)
## In this lengthy step, associate the correct beacon to the correct recv at that given time, this allows for inseason changes
...
## Now remove days when there wasn't a full day of beacon yet
...
beacon_by_day <- beacon_by_day[beacon_by_day$TagCode == beacon_by_day$beacon,]
## Remove detections at receivers that didn't have beacons yet but detected other's beacons
beacon_by_day <- beacon_by_day[!(beacon_by_day$recv == 17140 & beacon_by_day$day < as.Date("2018-06-19")),]
beacon_by_day <- beacon_by_day[!(beacon_by_day$recv == 17142 & beacon_by_day$day < as.Date("2018-06-19")),]
## now put in NAs for days when no beacons were deployed at receivers
beacon_by_day_add <- as.data.frame(rbind(cbind(17139, seq.Date(from = as.Date("2018-04-16"),to = as.Date("2018-06-18"), by = "day")),
                                         cbind(17136, seq.Date(from = as.Date("2018-04-16"),to = as.Date("2018-06-18"), by = "day")),
                                         cbind(17140, seq.Date(from = as.Date("2018-04-17"),to = as.Date("2018-06-18"), by = "day")),
                                         cbind(17142, seq.Date(from = as.Date("2018-04-17"),to = as.Date("2018-06-18"), by = "day"))))
## Massage these records for binding with beacon_by_day
colnames(beacon_by_day_add) <- c("recv", "day")      
beacon_by_day_add$recv <- as.character(beacon_by_day_add$recv)
beacon_by_day_add$day <- as.Date(beacon_by_day_add$day, origin = "1970-01-01")
...
gap_percent <- merge(gap_percent, aggregate(list(day_count = beacon_by_day$day), by = list(recv = beacon_by_day$recv), FUN = length), by = "recv", all.x = T)
...
beacon_by_day[as.Date(beacon_by_day$day) == as.Date(format(Sys.time(),tz="Etc/GMT+8")),"expected"] <- period_to_seconds(hms(format(floor_date(Sys.time(), unit = "hour"),tz="Etc/GMT+8", format = "%H:%M:%S")))/beacon_by_day[as.Date(beacon_by_day$day) == as.Date(format(Sys.time(),tz="Etc/GMT+8")),"beacon_PRI"]
...
efficiency <- aggregate(list(all_time_efficiency = beacon_by_day$day_efficiency), by = list(recv = beacon_by_day$recv), FUN = mean, na.rm = T)
...
for (k in unique(tagcodes$StudyID)){
  ...
  if (k == "Juv_Green_Sturgeon_2017"){
    filter_lag <- 0.1666667
    temp_detects$tag_life <- 150
  } else {
    filter_lag <- 0.08333333
  }
  ...
  if(nrow(temp_detects) > 0){
    ...
    temp_detects[is.na(temp_detects$time_to_next) == F & is.na(temp_detects$time_from_previous) == F & temp_detects$time_to_next < filter_lag & temp_detects$time_from_previous < filter_lag,"before_and_after_valid"] <- 1
...
map <- leaflet(data = gen_locs) %>%
    ...
    addMarkers(~longitude, ~latitude, label = ~general_location, group = "Sites") %>% 
...
kable(gen_locs[,c("recv", "location", "general_location", "latitude", "longitude", "rkm")], align = "c", row.names = F)
...
beacon_by_day <- merge(beacon_by_day, gen_locs[,c("recv", "location", "rkm")], by = "recv")
beacon_by_day <- beacon_by_day[order(beacon_by_day$rkm, decreasing = F),]
beacon_by_day$recv <- factor(beacon_by_day$recv, unique(beacon_by_day$recv))

par(mar=c(5.1,2.1,2.1,7.1))
plot(beacon_by_day$day, beacon_by_day$recv, yaxt = "n", xlab = "Total time receiver has been deployed and operational over whole study", pch = 15)
axis(4, labels = paste(unique(beacon_by_day$recv),unique(beacon_by_day$location), sep = " "), at = 1:length(unique(beacon_by_day$recv)), las = 2, cex.axis = 0.7)
points(as.Date(unique(beacon_by_day$recv_start, na.rm = T)), unique(beacon_by_day$recv), col = "red", pch = 15)
...
gen_loc_stats <- merge(gen_loc_stats, gap_percent[c("recv", "recv_start", "percent_downtime")], by = "recv", all.x = T)

gen_loc_stats <- merge(gen_loc_stats, efficiency, by = "recv", all.x= T)

gen_loc_stats <- gen_loc_stats[order(gen_loc_stats$rkm, decreasing = T),]

kable(gen_loc_stats[,c("recv", "location",  "recv_start", "percent_downtime", "all_time_efficiency", "last_download")], align = "c", row.names = F)
...
ggplot() +
  geom_tile(data=last_month, aes(x=factor(day), y=recv_lab, fill=day_efficiency, colour = "")) + 
