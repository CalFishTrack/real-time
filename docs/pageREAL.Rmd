...
beacon_by_day <- read.csv("beacon_by_day.csv", stringsAsFactors = F)
...
detects_beacon <- fread("C:/Users/Trailer/Desktop/Real-time data massaging/products/detects_beacon.csv", stringsAsFactors = FALSE)
...
if (nrow(ATS_files) > 0) {
  ...
  for (i in 1:nrow(downloads_new)){ 
    if(downloads_new[i, "filename"] %in% CST_files$Filenames | 
       downloads_new[i, "recv"] %in% c("17131","18007", "17134", "17145")) {
  ...
  for (i in 1:nrow(ATS_files)) {
    ...
    if (length(end_final) == 0 | file.info(ATS_files[i,"x"])$size == 0) {
      ...
      if(nrow(test4)>0){
        if(ATS_files[i,"x"] %in% CST_files$Filenames | 
           unique(test4$recv) %in% c("17131","17134","18007", "17145")) {
  ...
  if (length(detects_temp) > 0){
    ...
    detects_temp[detects_temp$recv %in% c(17127, 17139, 17135, 17143, 17147, 18008, 18003, 17144, 18002, 18006, 18007, 18092), "Temp"] <- NA
    
    ## Here, put only non-beacon detections in detection file
    detects_new <- detects_temp[!detects_temp$TagCode %in% c(unique(beacon_deployments$beacon), "FCAA", "006F", "006D"),]
...
## remove "day" from detects_beacon now that aggregate operation is over
detects_beacon$day <- NULL

beacon_by_day_new$day <- as.Date(beacon_by_day_new$day)
beacon_by_day_new$beacon <- as.character(NA)
...
for (i in 1:nrow(beacon_deployments)){
  beacon_by_day_new[which(beacon_deployments$recv[i] == beacon_by_day_new$recv & beacon_deployments$start[i] <= beacon_by_day_new$day & (beacon_deployments$stop[i] > beacon_by_day_new$day | is.na(beacon_deployments$stop[i]))),"beacon"] <- beacon_deployments$beacon[i]
}
## Now remove days when there wasn't a full day of beacon detections yet
beacon_by_day_new <- beacon_by_day_new[is.na(beacon_by_day_new$beacon)==F,]
## Now subset to only look at data for the correct beacon for that day
beacon_by_day_new <- beacon_by_day_new[beacon_by_day_new$TagCode == beacon_by_day_new$beacon,]
## Remove detections at receivers that didn't have beacons yet but detected other's beacons
#beacon_by_day_new <- beacon_by_day_new[!(beacon_by_day_new$recv == 17140 & beacon_by_day_new$day < as.Date("2018-06-19")),]
#beacon_by_day_new <- beacon_by_day_new[!(beacon_by_day_new$recv == 17142 & beacon_by_day_new$day < as.Date("2018-06-19")),]
...
#beacon_by_day_new <- bind_rows(beacon_by_day_new, beacon_by_day_add)
## Now bring in recv_start for next step
beacon_by_day_new <- merge(beacon_by_day_new, gap_percent[, c("location", "recv_start")])
...
beacon_by_day_new <- beacon_by_day_new[beacon_by_day_new$day > beacon_by_day_new$recv_start,]
...
## Make sure beacon_by_day$day and recv_start stays as a date/Posixct format, this can get stripped when beacon_by_day is reduced to zero rows
beacon_by_day$day <- as.Date(beacon_by_day$day)
beacon_by_day$recv_start <- as.POSIXct(beacon_by_day$recv_start)

## Now merge the two datasets together
beacon_by_day <- rbind(beacon_by_day, beacon_by_day_new)

## now write this new beacon_by_day file
write.csv(beacon_by_day, "beacon_by_day.csv", row.names = F)
...
if (nrow(downloads_new_all) > 0) {
  downloads_new_all <- merge(downloads_new_all, gen_locs[,c("recv", "location")], by = "recv")
...
kable(gen_loc_stats[,c("location",  "recv_start", "percent_downtime", "all_time_efficiency", "last_download")], align = "c", row.names = F)
