...
detects <- merge(detects, gen_locs,  by = "recv", all.x = TRUE)
detects_beacon <- merge(detects_beacon, gen_locs,  by = "recv", all.x = TRUE)
...
downloads <-  merge(downloads, gen_locs,  by = "recv", all.x = TRUE)
...
gap_percent <- aggregate(list(gap_count = gaps$gap_length_hours), by = list(recv = gaps$recv), FUN = sum) 
...
gap_percent$percent_files_missing <- round((gap_percent$gap_count / as.numeric(gap_percent$num_of_hours) * 100), 1)
...
#### (for now, this rough first cut says that a tag needs to have been detected at least 3 times in the 
#### whole array to have chances of being a true tag. This could likely be pushed upwards)
...
for (k in unique(tagcodes$StudyID)){
  ...
  if(nrow(temp_detects) > 0){
    ...
    ## Now remove RelDT column
    temp_detects <- temp_detects[, !names(temp_detects) == "RelDT"]
...
gen_locs <- merge(gen_locs, last_download, by = "location", all.x = T)

gen_locs <- gen_locs[order(gen_locs$last_download, decreasing = T),]

gen_locs <- merge(gen_locs, gap_percent[c("recv", "percent_files_missing")], by = "recv", all.x = T)
