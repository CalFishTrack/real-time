...
ATS_files <- as.data.frame(list(x=ATS_files[-grep(paste(bad_files$filename, collapse = "|"), ATS_files$x),]), stringsAsFactors = F)
...
detects <- detects[detects$time_from_previous > 1 | is.na(detects$time_from_previous) == TRUE ,]
...
for (k in unique(tagcodes$StudyID)){
  ...
  if(nrow(temp_detects) > 0){
    ...
    ## Now estimate the time in secs between the previous and next detection, for each detection.
    temp_detects$time_to_next <- as.numeric(difftime(shift(temp_detects$DateTime_Orig, fill = NA, type = "lead"), temp_detects$DateTime_Orig, units = "secs"))
    temp_detects$time_from_previous <- as.numeric(difftime(temp_detects$DateTime_Orig, shift(temp_detects$DateTime_Orig, fill = NA, type = "lag"),  units = "secs"))
    temp_detects$next_interval <- as.numeric(difftime(shift(temp_detects$DateTime_Orig, fill = NA, n = 2, type = "lead"), shift(temp_detects$DateTime_Orig, fill = NA, n = 1, type = "lead"), units = "secs"))
    temp_detects$prev_interval <- as.numeric(difftime(shift(temp_detects$DateTime_Orig, fill = NA, n = 1, type = "lag"), shift(temp_detects$DateTime_Orig, fill = NA, n = 2, type = "lag"), units = "secs"))
    ...
    # Is current detection within PRI*1.3*12+1 seconds from of the one after next?
    ...
    ## Now reorder the data by receiver to look at PRIs. That's because PRIs can only be accurately calculated using detection time on an individual receiver
    ## Also, to calculate PRI, we'll need to recalculate time to next and time from previous once we've reordered
    temp_detects <- temp_detects[order(temp_detects$visit_num, temp_detects$DateTime_PST),]
    ## Now estimate the time in secs between the previous and next detection, for each detection.
    temp_detects$time_to_next <- as.numeric(difftime(shift(temp_detects$DateTime_Orig, fill = NA, type = "lead"), temp_detects$DateTime_Orig, units = "secs"))
    temp_detects$time_from_previous <- as.numeric(difftime(temp_detects$DateTime_Orig, shift(temp_detects$DateTime_Orig, fill = NA, type = "lag"),  units = "secs"))
    temp_detects$time_to_2ndnext <- as.numeric(difftime(shift(temp_detects$DateTime_Orig, fill = NA, n = 2, type = "lead"), temp_detects$DateTime_Orig, units = "secs"))
    temp_detects$time_from_2ndprevious <- as.numeric(difftime(temp_detects$DateTime_Orig, shift(temp_detects$DateTime_Orig, fill = NA, n=2, type = "lag"),  units = "secs"))
    ...
    estPRI <- function(span = span, PRI = PRI){(span/PRI) / round(span/PRI, digits = 0) * PRI}
  
    ## If a middle detection from a ping train, look at before and after
    temp_detects$two_good_PRIS <- apply(apply(temp_detects[,c("time_from_previous", "time_to_next")], MARGIN = 2, function(x){abs(temp_detects$PRI_nominal - estPRI(span = x, PRI = temp_detects$PRI_nominal)) < temp_detects$PRI_nominal*0.11}), MARGIN = 1, FUN = sum)
    ## If first detection from a ping train, look at 2 next subsequent
    temp_detects[is.na(temp_detects$time_from_previous), "two_good_PRIS"] <- apply(apply(temp_detects[is.na(temp_detects$time_from_previous),c("time_to_2ndnext", "time_to_next")], MARGIN = 2, function(x){abs(temp_detects[is.na(temp_detects$time_from_previous),"PRI_nominal"] - estPRI(span = x, PRI = temp_detects[is.na(temp_detects$time_from_previous),"PRI_nominal"])) < temp_detects[is.na(temp_detects$time_from_previous),"PRI_nominal"]*0.11}), MARGIN = 1, FUN = sum)
    ## If last detection from a ping train, look at 2 previous
    temp_detects[is.na(temp_detects$time_to_next), "two_good_PRIS"] <- apply(apply(temp_detects[is.na(temp_detects$time_to_next),c("time_from_previous", "time_from_2ndprevious")], MARGIN = 2, function(x){abs(temp_detects[is.na(temp_detects$time_to_next),"PRI_nominal"] - estPRI(span = x, PRI = temp_detects[is.na(temp_detects$time_to_next),"PRI_nominal"])) < temp_detects[is.na(temp_detects$time_to_next),"PRI_nominal"]*0.11}), MARGIN = 1, FUN = sum)
    
    temp_detects <- temp_detects[which(temp_detects$two_good_PRIS == 2),]
  }
  
  ## Now remove a recv visit with less than 3 detections, now that we've removed some false detects
  if(nrow(temp_detects) > 0){  
    detects_per_visit_num <- aggregate(temp_detects$DateTime_Orig, by = list(visit_num = temp_detects$visit_num), FUN = length)
  ...
  fwrite(temp_detects, paste("C:/Users/field/Desktop/Real-time data massaging/products/Study_detection_files/", paste("detects",k,sep = "_"),".csv", sep = ""), row.names = F)
}
...
4. “Detection density filter”: Once valid fish visits have been determined and assigned to a specific fish, any 3 consecutive tag detections at an individual receiver must occur in a time window shorter than (Pulse Rate Interval (PRI) *1.3*12)+ 1 seconds. For example, for a 10 sec PRI, if a detection does not have at least 2 other detections within 157 seconds of it, it is removed. 


5. “PRI filter”: the time interval from the previous detection and the time interval to the next detection on an individual receiver must both be within 11% of the tag’s PRI for the detection to be valid. If the detection is the first of a receiver visit, then the time intervals to the subsequent two detections are used. If the detection is the last of a receiver visit, the time intervals from the previous two detections are used. 


6. “4-hit filter per general receiver location”: Once all the previous filters have been applied, many of which are being applied to detections on a per receiver basis, the final cut is that there must be 4 or more total detections per general location (which compiles data from multiple individual receivers).
...
volt.files$Datetime <- as.POSIXct(substr(volt.files$path, 41, stop = 55), format = "%Y%m%d-%H%M%S")
