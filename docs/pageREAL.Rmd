...
if (nrow(ATS_files) > 0) {
  ...
  try({
    for (i in 1:nrow(ATS_files)) {
      ...
      downloads_new <- rbind(downloads_new, 
                             cbind(filename = ATS_files[i, "shortname"],recv, hour))
    }
  })
  ...
  for (i in 1:nrow(downloads_new)){ 
    if(downloads_new[i, "filename"] %in% CST_files$Filenames | 
       downloads_new[i, "recv"] %in% c("17131","18007", "17134")) {
  ...
  short_files <- ATS_files[ATS_files$shortname %in% downloads_new[downloads_new$recv == "","filename"],]
  if(nrow(short_files)){  
    downloads_new <- downloads_new[!downloads_new$recv == "",]
  ...
  for (i in 1:nrow(ATS_files)) {
    ...
    if (length(end_final) == 0) {
      ...
      downloads_new <- downloads_new[!downloads_new$filename == ATS_files[i,"shortname"],]
      ...
      if(nrow(test4)>0){
        if(ATS_files[i,"x"] %in% CST_files$Filenames | 
           unique(test4$recv) %in% c("17131","17134","18007")) {
  ...
  if (length(detects_temp) > 0){
    ...
    detects_temp[detects_temp$recv %in% c(17127, 17139, 17135, 17143, 17147, 18008, 18003, 17144), "Temp"] <- NA
...
gap_percent[gap_percent$recv == "17134", "recv_start"] <- as.POSIXct("2018-05-22 16:00:00")
...
beacon_by_hour <- aggregate(detects_beacon1$DateTime_PST, by = list(recv = detects_beacon1$recv, hour = format(detects_beacon1$DateTime_PST, format = "%Y-%m-%d %H")), FUN = length)
...
## Also do per day
beacon_by_day <- aggregate(detects_beacon1$DateTime_PST, by = list(recv = detects_beacon1$recv, day = format(detects_beacon1$DateTime_PST, format = "%Y-%m-%d")), FUN = length)
beacon_by_day <- merge(beacon_by_day, gap_percent[, c("recv", "recv_start")])
beacon_by_day <- beacon_by_day[beacon_by_day$day > beacon_by_day$recv_start,]

gap_percent$num_of_hours <- round(difftime(Sys.time(), gap_percent$recv_start, units = "h"), 0)
## Now estimate the number of hours with no beacon detections
gap_percent$percent_files_missing <- 100-round(gap_percent$hour_count/as.numeric(gap_percent$num_of_hours)*100,1)
...
gap_percent[gap_percent$recv %in% c("17131", "17136", "17139", "17140", "17142"), "percent_files_missing"] <- 100-round(gap_percent[gap_percent$recv %in% c("17131", "17136", "17139", "17140", "17142"), "total_files"]/as.numeric(gap_percent[gap_percent$recv %in% c("17131", "17136", "17139", "17140", "17142"), "num_of_hours"])*100,1)

## remove the receivers without beacons
...
beacon_by_day <- merge(beacon_by_day,gen_locs[,c("recv", "beacon_PRI")], by = "recv", all.x = T)
...
beacon_by_day[as.Date(beacon_by_day$day) == as.Date(format(Sys.time(),tz="Etc/GMT+8")),"expected"] <- period_to_seconds(hms(format(Sys.time(),tz="Etc/GMT+8", format = "%H:%M:%S")))/beacon_by_day[as.Date(beacon_by_day$day) == as.Date(format(Sys.time(),tz="Etc/GMT+8")),"beacon_PRI"]
## Now estimate efficiency
beacon_by_day$day_efficiency <- round(beacon_by_day$x/beacon_by_day$expected,3)*100


## And for last week
last_week <- beacon_by_hour[which(as.POSIXct(beacon_by_hour$hour, format = "%Y-%m-%d %H") > Sys.time()-60*60*24*2),]

## Finally, make a table with all-time and last week's mean hourly detection efficiency
efficiency <- aggregate(list(all_time_efficiency = beacon_by_hour$hourly_efficiency), by = list(recv = beacon_by_hour$recv), FUN = mean)
efficiency <- merge(efficiency, aggregate(list(last_week_efficiency = last_week$hourly_efficiency), by = list(recv = last_week$recv), FUN = mean))
...
for (k in unique(tagcodes$StudyID)){
  ...
  temp_detects[temp_detects$PRI_nominal == 10, "tag_life"] <- 60
  temp_detects$tag_end <- temp_detects$RelDT + (temp_detects$tag_life*24*60*60)
  ...
  if(nrow(temp_detects) > 0){
    ...
    temp_detects$before_and_after_valid <- NA
    temp_detects[is.na(temp_detects$time_to_next) == F & is.na(temp_detects$time_from_previous) == F & temp_detects$time_to_next < 0.083 & temp_detects$time_from_previous < 0.083,"before_and_after_valid"] <- 1
    ...
    ## Add this data to study_detections file
    study_detections <- rbind(study_detections, cbind(k,temp_detects))
  }
...
write.csv(study_detections[, c("Study_ID", "TagCode", "DateTime_PST", "general_location", "latitude", "longitude", "rkm")], "study_detections.csv", row.names = F)
...
ATS_processed_files <- rbind(ATS_processed_files, ATS_files[names(ATS_processed_files)])
...
kable(gen_locs[,c("recv", "location", "general_location", "latitude", "longitude", "rkm")], align = "c")
...
Please note, receivers 17131, 17136, 17139, 17140, and 17142 are not displayed because they do not currently have beacon tags.
...
beacon_by_hour <- merge(beacon_by_hour, gen_locs[,c("recv", "location", "rkm")], by = "recv")
beacon_by_hour <- beacon_by_hour[order(beacon_by_hour$rkm, decreasing = F),]
beacon_by_hour$recv <- factor(beacon_by_hour$recv, unique(beacon_by_hour$recv))

par(mar=c(5.1,2.1,2.1,7.1))
plot(as.POSIXct(beacon_by_hour$hour, format = "%Y-%m-%d %H"), beacon_by_hour$recv, yaxt = "n", xlab = "Total time receiver has been deployed and operational over whole study")
axis(4, labels = paste(unique(beacon_by_hour$recv),unique(beacon_by_hour$location), sep = " "), at = 1:length(unique(beacon_by_hour$recv)), las = 2, cex.axis = 0.7)
points(unique(beacon_by_hour$recv_start), unique(beacon_by_hour$recv), col = "red", pch = 15)

```

<br/>

## Operational time for each receiver over last 7 days

<br/>

Please note, receivers 17131, 17136, 17139, 17140, and 17142 are not displayed because they do not currently have beacon tags.

```{r plot just last weeks receiver beacon detections, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 10, fig.width = 10}

## And for last week
last_week <- merge(last_week, gen_locs[,c("recv", "location", "rkm")], by = "recv")
last_week <- last_week[order(last_week$rkm, decreasing = F),]
last_week$recv <- factor(last_week$recv, unique(last_week$recv))

par(mar=c(5.1,2.1,2.1,7.1))
plot(as.POSIXct(last_week$hour, format = "%Y-%m-%d %H"), last_week$recv, yaxt = "n", xlab =  "Total time receiver has been deployed and operational over last week")
axis(4, labels = paste(unique(last_week$recv),unique(last_week$location), sep = " "), at = 1:length(unique(last_week$recv)), las = 2, cex.axis = 0.7)
points(unique(last_week$recv_start),unique(last_week$recv), col = "red", pch = 15)

```

<br/>

## All-time and last week's Detection Efficiency from Beacon Tags
...
gen_loc_stats <- merge(gen_locs[,c("recv", "location", "rkm")], last_download, by = "location", all.x = T)

gen_loc_stats <- gen_loc_stats[order(gen_loc_stats$rkm, decreasing = T),]

gen_loc_stats <- merge(gen_loc_stats, gap_percent[c("recv", "recv_start", "percent_files_missing")], by = "recv", all.x = T)

gen_loc_stats <- merge(gen_loc_stats, efficiency, by = "recv", all.x= T)

kable(gen_loc_stats[,c("recv", "location",  "recv_start", "percent_files_missing", "all_time_efficiency", "last_week_efficiency", "last_download")], align = "c", row.names = F)
...
last_month <- merge(last_month, gen_locs[,c("recv", "location", "rkm")], by = "recv")
...
last_month$recv_lab <- factor(last_month$recv_lab, unique(last_month$recv_lab))
## Now plot a heatmap of percent fish distribution per region, per timestep


ggplot(last_month, aes(x=day, y=recv_lab, fill=day_efficiency)) +
  geom_tile(colour = "white") + 
  scale_fill_gradient(low = "white", high = "red", name = "Daily Beacon Detection Efficiency") +
