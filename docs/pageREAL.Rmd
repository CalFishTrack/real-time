...
for (k in unique(tagcodes$StudyID)){
  ...
  if(nrow(temp_detects) > 0){
    ...
    ## Now redo the tag density (3 pings within PRI*1.3*12+1) since some pings will have been removed by the PRI filter above
  if(nrow(temp_detects) > 0){
    ## Now create two columns with time in hours between the previous and next detection, for each detection
    ## Now reorder the data since the above steps may have jumbled things
    temp_detects <- temp_detects[order(temp_detects$TagCode, temp_detects$recv, temp_detects$DateTime_PST),]
  ...
  ## Now remove less than 4 detects per visit
  if(nrow(temp_detects) > 0){  
  ...
  ## Now remove manually identified false detections that were missed by the filter
  ...
  if (nrow(temp_detects) == 0){
    temp_detects <- data.frame(Study_ID = studycodes$StudyID, TagCode = studycodes$TagID_Hex, location=NA,recv=NA,DateTime_Orig=NA, DateTime_PST=as.POSIXct(NA), general_location=NA, latitude=NA, longitude=NA, rkm=NA, stringsAsFactors = F)
  }else{
    temp_detects <- data.frame(Study_ID = k, temp_detects)
  }
  temp_detects <- merge(temp_detects, studycodes, by.x = c("Study_ID", "TagCode"), by.y = c("StudyID", "TagID_Hex"), all.y = T)

  study_detections <- rbindlist(list(study_detections, temp_detects))
  ...
  fwrite(temp_detects, paste("C:/Users/field/Desktop/Real-time data massaging/products/Study_detection_files/", paste("detects",k,sep = "_"),".csv", sep = ""), row.names = F, quote = T)
  rm(temp_detects)
}
...
```{r print heatmap of detection efficiency, warning=FALSE, message=FALSE, fig.width = 10}
...
```{r print voltage for ATS receivers,fig.height = 30, fig.width = 10, echo=FALSE, warning=FALSE, message=FALSE}
