...
TECKNO_all_files <- as.data.frame(list(x=dir(pattern = "*.txt", path = "C:/Users/Trailer/Data1/Dropbox (Teknologic LLC)/Apps/SRCOM/NOAA.SC", full.names = T, recursive = T)), stringsAsFactors = F)
...
TECKNO_files$dayes <- as.Date(apply(data.frame(TECKNO_files$x, stringsAsFactors = F), MARGIN = 1, FUN = substr, 82, 91), "%Y_%m_%d")
...
bad_files <- read.csv("C:/Users/Trailer/Desktop/Real-time data massaging/products/bad_files.csv", colClasses = "character")
...
UCD_all_files <- as.data.frame(list(x=dir(pattern = ".csv", path = "C:/Users/Trailer/Desktop/UCD_files", full.names = T)), stringsAsFactors = F)
...
if (nrow(UCD_files) > 0) {
  ...
  for (i in 1:nrow(UCD_files)) {
    ...
    if (inherits(res, "try-error") | ncol(res) != 3) {
      corrupted <- strsplit(UCD_files[i,], "/")
      corrupted2 <- unlist(lapply(corrupted, tail, 1))
      corrupted_downloads <- rbind(corrupted_downloads,cbind(V1 = corrupted2, download_attempts = 3, filename = UCD_files[i,]))
      ## Also make corrupted downloads more palateable
      next
    } else {
...
for (k in unique(tagcodes$StudyID)){
  ...
  if(nrow(temp_detects) > 0){
    ## Now remove any weird beacon tag echos, this was noticed for tag F694 at CVP_Tank1
    temp_detects <- temp_detects[!(temp_detects$TagCode == "F694" & temp_detects$general_location == "CVP_Tank1"),]
  }
  
  ## Now remove RelDT, PRI, tag_life and genloctag columns
  temp_detects <- temp_detects[, !names(temp_detects) %in% c("before_and_after_valid", "visit_num", "visit_step", "time_to_next", "time_from_previous", "prev_before_after_valid", "RelDT", "genloctag", "PRI_nominal", "tag_life", "tag_end")]
...
ggplot() +
  ...
  scale_fill_viridis(low = "red", high = "blue", name = "% Daily Beacon Detection Efficiency", na.value = "black", limits=c(0, 100)) +
