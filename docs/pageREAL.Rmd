...
## Now remove multipaths from detects_beacon for by day aggregation, raw data remains untouched
...
detects_beacon <- detects_beacon[order(detects_beacon$recv, detects_beacon$TagCode, detects_beacon$DateTime_PST),]
detects_beacon$time_from_previous <- as.numeric(difftime(detects_beacon$DateTime_PST, shift(detects_beacon$DateTime_PST, fill = NA, type = "lag"),  units = "secs"))
## Now make NA the time diff values when it's between 2 different gen locs or tagcodes
detects_beacon[which(detects_beacon$recv != shift(detects_beacon$recv, fill = NA, type = "lag")), "time_from_previous"] <- NA
detects_beacon[which(detects_beacon$TagCode != shift(detects_beacon$TagCode, fill = NA, type = "lag")), "time_from_previous"] <- NA
## Now remove the second detection every time the detection lag difftime is less than 5 sec (multipath and funny behavior)
temp_detects <- detects_beacon[detects_beacon$time_from_previous > 5 | is.na(detects_beacon$time_from_previous) == TRUE ,]
detects_beacon$time_from_previous <- NULL
...
temp_detects$day <- format(temp_detects$DateTime_PST, format = "%Y-%m-%d")
beacon_by_day_new <- temp_detects[,list(count = length(DateTime_PST)), by = list(recv,location,TagCode,day)]
...
beacon_by_day <- merge(beacon_by_day, gap_percent[, c("location", "recv_start")])
...
beacon_by_day[beacon_by_day$day_efficiency >100, "day_efficiency"] <- 100
