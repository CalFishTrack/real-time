...
for (k in unique(tagcodes$StudyID)){
  ...
  temp_detects <- temp_detects[, names(temp_detects) %in% c("TagCode" ,"location","recv","DateTime_Orig", "DateTime_PST", "tag_life", "general_location", "latitude", "longitude", "rkm")]
...
```{r print heatmap of detection efficiency, warning=FALSE, message=FALSE}
...
## Ensure that recvs without any detections still show up
last_month$location <- factor(last_month$location, levels = unique(gen_loc_stats[gen_loc_stats$Active == "Yes","location"]))
