...
detects_beacon$DateTime_PST <- as.POSIXct(detects_beacon$DateTime_PST, format = "%Y-%m-%d %H:%M:%OS")
...
if (nrow(ATS_files) > 0) {
  ...
  for (i in 1:nrow(ATS_files)) {
    ...
    if (length(end_final) == 0) {
      ...
      detects_temp <- rbind(detects_temp, test3)
    }
  }
  ...
  detects_temp <- detects_temp[!detects_temp$TagCode == " GPS Fix  ",]
  
  names(detects_temp)[names(detects_temp) == 'DateTime'] <- 'DateTime_Orig'
  
  detects_temp$DateTime_Orig <- as.POSIXct(detects_temp$DateTime_Orig, format = "%m/%d/%Y %H:%M:%OS", tz = "Etc/GMT+8")
  
  #attributes(detects_temp$DateTime_PST)$tzone <- "Etc/GMT+8"
  
  detects_temp$DateTime_PST <- detects_temp$DateTime_Orig
  
  ## change Datetime_Orig with milliseconds shown (this turns the class into character)
  detects_temp$DateTime_Orig <- format(detects_temp$Orig, "%Y-%m-%d %H:%M:%OS6")
  ...
  detects_new <- detects_temp[!detects_temp$TagCode %in% c("FA54", "75D3", "FA96"),]
  ## And put beacon detections into seperate file
  detects_beacon_new <- detects_temp[detects_temp$TagCode %in% c("FA54", "75D3", "FA96"),]

  #detects_new <- merge(detects_new, coords, by = "recv", all.x = T)
  
  #detects_new <- merge(detects_new, gen_locs, by = "recv", all.x = T)
  
  detects_beacon <- rbind(detects_beacon, detects_beacon_new[,c("recv", "DateTime_Orig", "TagCode", "DateTime_PST", "location", "general_location", "latitude", "longitude", "rkm", "Temp")])
  detects <- rbind(detects, detects_new[,c("recv", "DateTime_Orig", "TagCode", "DateTime_PST", "location", "general_location", "latitude", "longitude", "rkm", "Temp")])
...
if (nrow(TECKNO_files) > 0) {
  ...
  detects_temp <- merge(detects_temp, gen_locs[,c("recv", "location", "general_location", "latitude", "longitude", "rkm")])
  ## Add column for temperature
  detects_temp$Temp <- NA
  
  ## Here, put only non-beacon detections in detection file
  detects_new <- detects_temp[!detects_temp$TagCode %in% c("FADA", "F96A", "FCAA", "FA55", "006F", "006D"),]
  ## And put beacon detections into seperate file
  detects_beacon_new <- detects_temp[detects_temp$TagCode %in% c("FADA", "F96A", "FCAA", "FA55", "006F", "006D"),]
...
#### As a preliminary cut to detection filter, here subset for only tags known to be in the system 
...
#### Now, subset for only tags with 2 or more detects per gen loc within 24 hours, by studyID
