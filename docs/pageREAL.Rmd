...
## These should never happen since there are so many false detections

res <- aggregate(detects$TagCode,
                 by=list(detects$recv, format(detects$DateTime_PST, "%Y-%m-%d %H")),
                 length)

colnames(res) <- c("recv", "DateTimeHour_PST", "detect_count")

hours <- as.data.frame(format(seq(
          from= min(detects$DateTime_PST)+60*60*24,
...
daily_beacon <- aggregate(list(tag_count = detects_beacon$location), by = list(recv = detects_beacon$recv, day = detects_beacon$Day, tag = detects_beacon$TagCode), FUN = length)

daily_beacon$expected_count <- 24*60
