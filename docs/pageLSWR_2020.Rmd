...
Telemetry Study Template for this study can be found [here](https://github.com/CalFishTrack/real-time/blob/master/data/Telemetry_Study_Summary_NOAA_LSNFH_Winter-run.pdf?raw=true)
...
detects_study <- as.data.frame(detects_study[detects_study$Study_ID == "Winter_H_2020",])
detects_study$DateTime_PST <- as.POSIXct(detects_study$DateTime_PST, format = "%Y-%m-%d %H:%M:%S", "Etc/GMT+8")
detects_study$release_time <- as.POSIXct(detects_study$RelDT, format = "%Y-%m-%dT%H:%M:%S")
...
##################################################################################################################
#### TO RUN THE FOLLOWING CODE CHUNKS FROM HERE ON DOWN USING R ERDDAP, UN-COMMENT THESE NEXT 4 LINES OF CODE ####
##################################################################################################################
#cache_delete_all()
#detects_study <- tabledap('FEDcalFishTrack', url = "https://oceanview.pfeg.noaa.gov/erddap/", 'Study_ID="Winter_H_2020"')
#detects_study$DateTime_PST <- as.POSIXct(detects_study$local_time, format = "%Y-%m-%d %H:%M:%S", "Etc/GMT+8")
#detects_study$release_time <- as.POSIXct(detects_study$release_time, format = "%Y-%m-%dT%H:%M:%S")
#detects_study$river_km <- as.numeric(detects_study$river_km)
##################################################################################################################

detects_study$Release <- "Release 1"
detects_study[detects_study$release_time > as.POSIXct("2020-03-20"), "Release"] <- "Release 2"
...
if (nrow(study_tagcodes) == 0){
  ...
  cat(paste("Project began on ", min(study_tagcodes$release_time), ", see tagging details below:", sep = ""))
...
***
***Data current as of `r anicon::nia(read.csv(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products\\latest_download.csv", sep = "")), size = 1, colour = "red")`. Updates occur hourly. All times in Pacific Standard Time.***

```{r print figure of fish detections at Butte, message = FALSE, fig.height = 6, fig.width = 10, out.extra='style="background-color: #000000; padding:3px"', fig.cap="2.1 Detections at Butte City Bridge versus Sacramento River flows at Bend Bridge"}
...
if (nrow(detects_butte) == 0){
  ...
  endtime <- min(as.Date(format(Sys.time(), "%Y-%m-%d")), max(as.Date(detects_butte$release_time)+(as.numeric(detects_butte$tag_life)*1.5)))
...
if (nrow(detects_tower) == 0){
  ...
  inp <- as.data.frame(dcast(test, TagCode ~ river_km, fun.aggregate = length))
...
if (nrow(detects_benicia) == 0){
  ...
  inp <- as.data.frame(dcast(test3, TagCode ~ river_km, fun.aggregate = length))
...
figheight <- max(c(3,as.numeric(difftime(Sys.Date(), min(study_tagcodes$release_time), units = "days")) / 4))
...
if (nrow(detects_study) == 0){
 ...
 kable(crosstab, align = "c") %>%
  kable_styling(c("striped", "condensed"), font_size = 11) %>%
  #row_spec(0, angle = -45) %>%
  column_spec(column = 1:12,width_min = "50px",border_left = T, border_right = T) %>%
