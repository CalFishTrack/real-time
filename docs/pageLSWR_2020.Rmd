...
#study_tagcodes <- tagcodes[tagcodes$StudyID == "Winter_H_2020",]
...
```{r print figure of fish detections at Butte, message = FALSE, fig.height = 6, fig.width = 10, out.extra='style="background-color: #000000; padding:3px"', fig.cap="2.1 Detections at Butte City Bridge versus Sacramento River flows at Butte City"}
...
if (nrow(detects_butte) == 0){
  ...
  endtime <- min(as.Date(format(Sys.time(), "%Y-%m-%d")), max(as.Date(detects_butte$release_time)+(detects_butte$tag_life*1.5)))
...
if (nrow(detects_tower) == 0){
  ...
  ## Endtime should be either now, or end of predicted tag life, whichever comes first
  endtime <- min(as.Date(format(Sys.time(), "%Y-%m-%d")), max(as.Date(detects_tower$release_time)+(detects_tower$tag_life*1.5)))
