...
```{r print figure of fish detections at Butte, message = FALSE, fig.height = 6, fig.width = 10, out.extra='style="background-color: #9ecff7; padding:10px"', fig.cap="2.1 Detections at Butte City Bridge versus Sacramento River flows at Butte City"}
...
detects_study <- fread(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products\\Study_detection_files\\detects_Winter_H_2020.csv", sep = ""))
...
```{r print figure of fish detections at Tower, message = FALSE, fig.height = 6, fig.width = 10, out.extra='style="background-color: #9ecff7; padding:10px"',fig.cap="2.2 Detections at Tower Bridge (downtown Sacramento) versus Sacramento River flows at Wilkins Slough"}
...
```{r print figure of fish detections at Benicia, message = FALSE, fig.height = 6, fig.width = 10, out.extra='style="background-color: #9ecff7; padding:10px"', ,fig.cap="2.3 Detections at Benicia Bridge"}
...
if(nrow(test2[test2$general_location %in% c("Sac_BlwGeorgiana", "Sac_BlwGeorgiana2"),]) == 0 |
  ...
  mtext(text = "3.3 Routing Probabilities at Georgiana Slough Junction", cex = 1.3, side = 1, line = 0.2, adj = 0)
...
figheight <- max(c(3,as.numeric(difftime(Sys.Date(), min(study_tagcodes$RelDT), units = "days")) / 5))
