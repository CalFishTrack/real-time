...
if (nrow(study_tagcodes) == 0){
  ...
  study_tagcodes$Release <- "March release"
  study_tagcodes[study_tagcodes$RelDT > as.POSIXct("2020-04-20"), "Release"] <- "May release"
...
```{r print figure of fish detections at Butte, message = FALSE, warning = F, fig.height = 6, fig.width = 10, out.extra='style="background-color: #000000; padding:3px"', fig.cap="2.1 Detections at Butte City Bridge versus Sacramento River flows at Bend Bridge"}
...
detects_study <- fread(paste(file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\"), "\\Real-time data massaging\\products\\Study_detection_files\\detects_BC_Jumpstart_2020.csv", sep = ""), colClasses = c(DateTime_PST = "character", RelDT = "character"))
