...
study_tagcodes <- tagcodes[tagcodes$StudyID == "FR_Spring_2019",]
...
if (nrow(study_tagcodes) == 0){
  ...
  study_tagcodes$Release <- "Boyds"
  study_tagcodes[study_tagcodes$RelDT < as.POSIXct("2019-04-22 12:00:00"), "Release"] <- "Gridley"
...
detects_study <- read.csv("detects_FR_Spring_2019.csv", stringsAsFactors = F)
...
if (nrow(detects_study) == 0){
  ...
  if(nrow(test2[test2$general_location %in% c("Sac_BlwGeorgiana", "Sac_BlwGeorgiana2"),]) == 0 |
    ...
    georg <- rbind(georg, cbind(tail(results_short,1), latest, unique(study_tagcodes$StudyID)))
    
    write.csv(georg, "daily_georg_routing.csv", row.names = F)
  ...
  for (j in sort(unique(study_tagcodes$Release))) {
    ...
    if(nrow(detects_study[detects_study$Release == j,]) > 0 ) {
      ...
      final_stats <- kable(tag_stats1, row.names = F, 
            caption = paste("Detections for",j,"release group", sep = " "),
            "html")
