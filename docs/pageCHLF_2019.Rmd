...
study_tagcodes <- tagcodes[tagcodes$StudyID == "ColemanLateFall_2019",]
...
detects_study <- read.csv("detects_ColemanLateFall_2019.csv", stringsAsFactors = F)
...
if (nrow(detects_benicia)>0) {
  ...
  rels <- unique(study_tagcodes[study_tagcodes$StudyID == unique(detects_benicia$StudyID), "RelDT"])
  rel_num <- length(rels)
  rels_no_detects <- as.character(rels[!(rels %in% unique(detects_benicia$RelDT))])
  
  tagcount <- aggregate(list(unique_tags = detects_benicia$TagCode), by = list(Day = detects_benicia$Day, RelDT = detects_benicia$RelDT ), FUN = function(x){length(unique(x))})
...
if (nrow(detects_study) == 0){
  ...
  if(nrow(test2[test2$general_location %in% c("Sac_BlwGeorgiana", "Sac_BlwGeorgiana2"),]) == 0 |
    ...
    study_tagcodes[study_tagcodes$last_location %in% c("Georgiana_Slough1", "Georgiana_Slough2"), "inp_final"] <- paste("A",study_tagcodes[study_tagcodes$last_location %in% c("Georgiana_Slough1", "Georgiana_Slough2"), "inp_final"]inp_part1, study_tagcodes$inp_partB," 1 ;", sep = "")
