...
if (nrow(detects_study) == 0){
  ...
  detects_tower <- merge(detects_tower, study_tagcodes[,c("TagID_Hex", "RelDT", "StudyID", "Release_week")], by.x = "TagCode", by.y = "TagID_Hex")
  
  starttime <- as.Date(min(detects_tower$RelDT), "Etc/GMT+8")
  endtime <- as.Date(c(Sys.time()))#, max(detects_tower$first_detect)+60*60*24)))
...
if (nrow(detects_benicia)>0) {
  ...
  detects_benicia <- merge(detects_benicia, study_tagcodes[,c("TagID_Hex", "RelDT", "StudyID")], by.x = "TagCode", by.y = "TagID_Hex")
...
test <- fread("detects_RBDD-2018.csv", stringsAsFactors = FALSE)
gen_locs <- read.csv("realtime_locs.csv", stringsAsFactors = F)
